(function(){
  'use strict';
  var App={state:null,ui:null,step:1,replaceIndex:null,waitTimer:null,progressTimer:null,terminalTimer:null,galleryIndex:0,savedPhotoSets:[]};
  ...
  function toast(msg){var t=$('#toast');t.textContent=msg;t.classList.add('show');setTimeout(function(){t.classList.remove('show')},5200)}
  function gotoStep(n){App.step=n;$$('.stage').forEach(function(el){el.classList.toggle('active',parseInt(el.getAttribute('data-step'),10)===n)});$$('[data-step-nav]').forEach(function(el){var x=parseInt(el.getAttribute('data-step-nav'),10);el.classList.toggle('active',x===n);el.classList.toggle('done',x<n)});window.scrollTo({top:0,behavior:'smooth'});}
  ...
  function renderUploads(){
    ...
    uploads.forEach(function(u,i){
      ...
      card.querySelector('.replace').onclick=function(){App.replaceIndex=i;$('#photo-input').click()};
      card.querySelector('.remove').onclick=function(){var fd=new FormData();fd.append('index',i);api('remove_upload',{method:'POST',body:fd}).then(function(r){App.state=r.state;renderUploads();renderPhotoSetHistory()}).catch(function(e){toast(e.message)})};
  ...
  function refreshSavedPhotoSets(openAfter){return api('list_saved_photosets').then(function(r){renderSavedPhotoSets(r.photosets||[]);if(openAfter)openSavedPhotoSets();return r}).catch(function(e){if(openAfter)toast(e.message)})}
  ...
  function pollPhotoSet(){
    api('poll_photoset',{strict:false}).then(function(r){if(r.ok&&r.status==='succeeded'){App.state=r.state;finishWaitWithDecrypt('photoset',function(){renderPhotoSet(App.state.photoset_images||r.images,true);renderPhotoSetHistory();gotoStep(2)})}else if(r.ok){setTimeout(pollPhotoSet,1800)}else{stopWait();toast(r.message||'FotoSet-Generierung fehlgeschlagen.')}}).catch(function(e){stopWait();toast(e.message)});
  }
  function pollScenes(){
  ...
  document.addEventListener('DOMContentLoaded',function(){matrix($('#matrix-bg'),false);matrix($('#wait-matrix'),true);bind();refreshSavedPhotoSets(false);api('bootstrap').then(configureUi).catch(function(e){$('#system-label').textContent='CONFIG REQUIRED';toast(e.message);api('status').then(function(r){App.state=r.state;renderUploads();renderPhotoSetHistory();gotoStep(1)}).catch(function(){})})});
})();

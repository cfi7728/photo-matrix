(function(){
  ...
  function pollPhotoSet(){
    api('poll_photoset',{strict:false}).then(function(r){if(r.ok&&r.status==='succeeded'){App.state=r.state;finishWaitWithDecrypt('photoset',function(){renderPhotoSet(App.state.photoset_images||r.images,true);renderPhotoSetHistory();gotoStep(2)})}else if(r.status==='succeeded_storage_failed'){stopWait();if(r.state)App.state=r.state;toast(r.message||'Das FotoSet konnte nicht gespeichert werden.','SPEICHERN WIEDERHOLEN',function(){startWait('photoset');pollPhotoSet()})}else if(r.ok){setTimeout(pollPhotoSet,1800)}else{stopWait();toast(r.message||'FotoSet-Generierung fehlgeschlagen.')}}).catch(function(e){stopWait();toast(e.message)});
  }
  function pollScenes(){

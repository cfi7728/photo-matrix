(function(){
  'use strict';
  var App={state:null,ui:null,step:1,isTransitioning:false,hasBootstrapped:false,lastInputWasKeyboard:false,replaceIndex:null,waitTimer:null,progressTimer:null,terminalTimer:null,galleryIndex:0,galleryLoadId:0,gallerySwapTimer:null,savedPhotoSets:[],savedPhotoSetsRefreshId:0,modalBackgroundState:null};
  ...
  function openGallery(index,trigger){var items=galleryItems();if(!items.length)return;App.galleryIndex=typeof index==='number'?index:0;mountModal($('#gallery-modal'),trigger,$('#gallery-x'));updateGallery()}
  function closeGallery(){clearTimeout(App.gallerySwapTimer);App.galleryLoadId++;unmountModal($('#gallery-modal'))}
